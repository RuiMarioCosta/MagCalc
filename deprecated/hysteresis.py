...
def free_energy_intersections(sigma, T, B, *args):
    ...
    if not args:  # if the free energies weren't calculated
        diff = free.F_tot_vs_M(sigma, T, B, J1, TC1, lamb1, theta_D1, F01) - free.F_tot_vs_M(sigma, T, B, J2, TC2,
                                                                                             lamb2, theta_D2, F02)
...
def norm_factor(sigma, T, B, *args):
    ...
    if len(min_indices) == 1:  # if there is only 1 minimum
        ...
        int_mid = simps(fm[max_index_left:max_index_right], sigma[max_index_left:max_index_right],
                        dsigma)  # middle normalizing factor
        ...
        int_left = simps(fm[:max_index_left], sigma[:max_index_left], dsigma)  # left nomalizing factor
        int_right = simps(fm[max_index_right:], sigma[max_index_right:], dsigma)  # right normalizing factor
        int_mid_left = simps(fm[max_index_left:max_index_mid], sigma[max_index_left:max_index_mid],
                             dsigma)  # middle left normalizing factor
        int_mid_right = simps(fm[max_index_mid:max_index_right], sigma[max_index_mid:max_index_right],
                              dsigma)  # middle right normalizing factor
...
def diff_eq(z, t):
    ...
    if len(min_indices) == 3:  # if there are 3 minimums
        ...
        if intersections == []:  # if the free energies don't intersect it means that the two minimums belong to the high magnetization states of the phase with highest Curie temperature
            ...
            elif (a < min_indices[0] < b) and min_indices[
                1] > b:  # if the one minimum belongs to the low mag. state and the other to the positive high mag. state
            ...
            elif min_indices[0] < a and (a < min_indices[
                1] < b):  # if the one minimum belongs to the low mag. state and the other to the negative high mag. state
...
def frac_and_M(t):
    ...
    if len(min_indices) == 2:  # if there are 2 minimums put 50% in each minimum
        ...
        if intersections == []:  # if the curves of free energy do not intersect then the 2 minimums belong to the phase with highest Curie temperature (phase 2)
            ...
            if min_indices[0] < a and min_indices[
                1] > b:  # if both minimus belong to the positive and negative ferromagnetic state of phase 2
                z0 = [0.5, 0., 0., 0., 0.5]
            elif (a < min_indices[0] < b) and min_indices[
                1] > b:  # if one minimum is from the paramagnetic state of phase 1 and the other from the positive ferromagnetic state of phase 2
                z0 = [0., 0., 0.5, 0., 0.5]
            elif min_indices[0] < a and (a < min_indices[
                1] < b):  # if one minimum is from the paramagnetic state of phase 1 and the other from the negative ferromagnetic state of phase 2
                z0 = [0.5, 0., 0.5, 0., 0.]
    elif len(
            min_indices) == 4:  # if there are 4 minimums then they are all from positive and negative ferromagnetic states of both structures
        z0 = [0.25, 0.25, 0., 0.25, 0.25]
    elif len(
            min_indices) == 3:  # if there are 3 minimums then, at B=0, they belong to the positive and negative ferromagnetic states of phase 2 and to the paramagnetic state of phase 1
    ...
    for i in range(len(t)):  # for every point in time
        ...
        if len(min_indices) == 1:  # if there is only 1 minimum
            ...
            if T[
                i] >= TC2:  # if the temperature if above the highest Curie temperature then the minimum is from the paramagnetic state of phase 1
                sig1_mid = sig[
                    argrelmin(fm)[0]]  # value of the magnetization at the given temperature and magnetic field
                M[i] = sig1_mid * z1_mid[i]  # magnetization at index i
            elif T[i] < TC2 and B[
                i] > 0.:  # if the temperature if below the highest Curie temperature and B>0 then the minimum is from the positive ferromagnetic state of phase 2
                sig2_right = sig[
                    argrelmin(fm)[0]]  # value of the magnetization at the given temperature and magnetic field
                M[i] = sig2_right * z2_right[i]  # magnetization at index i
            elif T[i] < TC2 and B[
                i] < 0.:  # if the temperature if below the highest Curie temperature and B<0 then the minimum is from the negative ferromagnetic state of phase 2
                sig2_left = sig[
                    argrelmin(fm)[0]]  # value of the magnetization at the given temperature and magnetic field
            ...
            if intersections == []:  # if there are nointersections
                ...
                if min_indices[0] < a and min_indices[
                    1] > b:  # if both minimus belong to the positive and negative ferromagnetic state of phase 2
                    sig2_left, sig2_right = sig[
                        argrelmin(fm)[0]]  # magnetizations of both minimums at the given T and B
                    M[i] = sig2_right * z2_right[i] + sig2_left * z2_left[i]  # magnetization at index i
            ...
            if T[i] >= TC1:  # if it's above the Curie temperature then it will only be z1_mid
                sig2_left, sig1_mid, sig2_right = sig[
                    argrelmin(fm)[0]]  # magnetizations of minimums at the given T and B
                M[i] = sig1_mid * z1_mid[i] + sig2_right * z2_right[i] + sig2_left * z2_left[
                    i]  # magnetization at index i

            elif T[i] < TC1 and B[
                i] > 0.:  # if the temperature if below the highest Curie temperature and B>0 then the minimum is from the positive ferromagnetic state of phase 2
                sig2_left, sig1_right, sig2_right = sig[
                    argrelmin(fm)[0]]  # magnetizations of minimums at the given T and B
                M[i] = sig1_right * z1_right[i] + sig2_right * z2_right[i] + sig2_left * z2_left[
                    i]  # magnetization at index i

            elif T[i] < TC1 and B[
                i] < 0.:  # if the temperature if below the highest Curie temperature and B<0 then the minimum is from the negative ferromagnetic state of phase 2
                sig2_left, sig1_left, sig2_right = sig[
                    argrelmin(fm)[0]]  # magnetizations of minimums at the given T and B
                M[i] = sig1_left * z1_left[i] + sig2_right * z2_right[i] + sig2_left * z2_left[
                    i]  # magnetization at index i

        elif len(min_indices) == 4:  # if there are 4 minimums
            sig2_left, sig1_left, sig1_right, sig2_right = sig[
                argrelmin(fm)[0]]  # magnetizations of minimums at the given T and B
            M[i] = sig2_left * z2_left[i] + sig1_left * z1_left[i] + sig1_right * z1_right[i] + sig2_right * z2_right[
                i]  # magnetization at index i
    ...
    if do_plots == True:
        ...
        if do_hys == True:
            ...
            if save == True:
                ...
                np.savetxt("Hysteresis\Hysteresis(T=" + str(T0) + "K, B=" + str(B0) + "T).txt", zipped, delimiter=',',
                           header='Time, Temperature, Magnetic Field, z2_left, z1_left, z1_mid, z1_right, z2_right, Reduced Magnetization', )
