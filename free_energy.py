...
def transition_temp(T, B, J1, TC1, theta_D1, F01, lamb1,
    ...
    for j in range(T.shape[0]):
        for i in range(T.shape[1] - 1):
            if diff[j,i] == 0. or diff[j,i] * diff[j,i + 1] < 0.:
                # crossover at i
                Ts[j] = T[0,i]
...
def F_stable(T, B, J1, J2, gJ, TC1, TC2, lamb1, lamb2, Nm, theta_D1, theta_D2,
             N, F01, F02):
...
def F_M_vs_M(sigma, T, B, J, gJ, TC, lamb, Nm):
    ...
    sigma0 = mag.Brillouin(T, B, J, gJ, TC, lamb, Nm) # reduced magnetization of average minimum
    
    h = B/(lamb*Nm*gJ*mu_B*J) # relative field to the saturation magnetization
    y = 3.*J/(J+1.)*(h + sigma0)*TC/T
    C = np.sinh((2.*J + 1.)*y/(2.*J))
    D = np.sinh(y/(2.*J))
    F0 = -k_B*T*np.log(C/D) # average free energy
...
def F_totstable_vs_M(sigma, T, B, J1, TC1, lamb1, theta_D1, F01,
                     J2, TC2, lamb2, theta_D2, F02, gJ, Nm, N, *args):
...
if __name__ == "__main__":
    ...
    cProfile.runctx('F_totstable_vs_M(sigma,T,B)', {'F_totstable_vs_M':F_totstable_vs_M,'sigma':sig, 'T': T0, 'B': B0}, {}, filename = file_name)
    ...
    ax1.plot(sig, F_M_vs_M(sig, T0, 0, J1, TC1, lamb1), label='$F^{M}$')
    ax1.plot(sig, F_M_vs_M(sig, T0, 0, J2, TC2, lamb2), label='$F^{M}$')
    ax1.set_title('T=%f K, B=%d T' % (T0,0))
    
    ax2.plot(sig, F_tot_vs_M(sig, T0, 0, J1, TC1, lamb1, theta_D1, F01),label='$F^{tot}1$')
    ax2.plot(sig, F_tot_vs_M(sig, T0, 0, J2, TC2, lamb2, theta_D2, F02),label='$F^{tot}2$')
    ax2.plot(sig, F_totstable_vs_M(sig, T0, 0), label='$F^{Tot stable}$')
    ...
    ax1.legend(loc=0)
    ax2.legend(loc=0)
    ax2.set_xlabel('Reduced Magnetization, $\sigma$')

    
    f, (ax1, ax2) = plt.subplots(2, sharex=True)
    ax1.plot(sig, F_M_vs_M(sig, T0, B0, J1, TC1, lamb1), label='$F^{M}$')
    ax1.plot(sig, F_M_vs_M(sig, T0, B0, J2, TC2, lamb2), label='$F^{M}$')
    ax1.set_title('T=%f K, B=%d T' % (T0,B0))
    
    ax2.plot(sig, F_tot_vs_M(sig, T0, B0, J1, TC1, lamb1, theta_D1, F01),label='$F^{tot}1$')
    ax2.plot(sig, F_tot_vs_M(sig, T0, B0, J2, TC2, lamb2, theta_D2, F02),label='$F^{tot}2$')
    ax2.plot(sig, F_totstable_vs_M(sig, T0, B0), label='$F^{Tot stable}$')
