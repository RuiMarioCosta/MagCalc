...
def plt_M_vs_T(T, B, sigma_1, sigma_2, sigma_stable, Bstep=1, save=False):
    ...
    if B.shape == ():  # if B is a scalar
        ...
        if save == True:
            ...
            np.savetxt("M_vs_T\M_vs_T(" + str(B) + "T).txt", zipped, delimiter=',',
                       header='Temperature, Magnetization 1, Magnetization 2, Stable Magnetization',)
        ...
        for i in range(0, len(B), Bstep):
            ...
            if save == True:
                ...
                np.savetxt("M_vs_T\M_vs_T(" + str(B[i]) + "T).txt", zipped, delimiter=',',
                           header='Temperature, Magnetization 1, Magnetization 2, Stable Magnetization',)
...
def plt_M_vs_B(T, B, sigma_1, sigma_2, sigma_stable, Tstep=1, save=False):
    ...
    if T.shape == ():  # if T is a scalar
        ...
        if save == True:
            ...
            np.savetxt("M_vs_B\M_vs_B(" + str(T) + "K).txt", zipped, delimiter=',',
                       header='Magnetic Field, Magnetization 1, Magnetization 2, Stable Magnetization',)
        ...
        for i in range(0, len(T), Tstep):
            ...
            if save == True:
                ...
                zipped = zip(B, sigma_1[:, i], sigma_2[
                             :, i], sigma_stable[:, i])
                np.savetxt("M_vs_B\M_vs_B(" + str(T[i]) + "K).txt", zipped, delimiter=',',
                           header='Magnetic Field, Magnetization 1, Magnetization 2, Stable Magnetization',)
...
def plt_M_vs_TB(T, B, sigma_stable,):
    """Plots the stable magnetization as a function of temperature and magnetic field.
...
def plt_U_vs_T(T, B, sigma_1, sigma_2, Bstep=1, save=False,
    ...
    """Plots the internal energy of both structures as a function of temperature.
    ...
    for i in range(0, len(B), Bstep):
        ...
        if save == True:
            ...
            np.savetxt("U_vs_T\U_vs_T(" + str(B[i]) + "T).txt", zipped, delimiter=',', header='Temperature, Internal Energy 1, Internal Energy 2',)
...
def plt_M_hys_vs_T(T, B, sigma_heat, sigma_cool, Bstep=1, save=False):
    ...
    for i in range(0, len(B), Bstep):
        ...
        if save == True:
            ...
            np.savetxt("M_hys_vs_T\M_hys_vs_T(" + str(B[i]) + "T).txt", zipped, delimiter=',',
                       header='Temperature, Magnetization Heating, Magnetization Cooling',)
...
def plt_M_hys_vs_B(T, B, sigma_heat, sigma_cool, Tstep=1, save=False):
    ...
    for j in range(0, len(T), Tstep):
        ...
        if save == True:
            ...
            np.savetxt("M_hys_vs_B\M_hys_vs_B(" + str(T[j]) + "T).txt", zipped, delimiter=',',
                       header='Magnetic Field, Magnetization Heating, Magnetization Cooling',)
...
def plt_M_hys_vs_TB(T, B, sigma_heat, sigma_cool):
    """Plots the magnetization on heating and cooling as a function of temperature 
    and magnetic field.
...
def plt_Gauss(T, TC1, Var1, TC2, Var2):
    """Plots the gaussian distribution.

    Parameters
    ---------
    T : array
        Temperatures. The array must include both Curie temperatures.
    TC : scalar
        Average Curie temperature.
    Var : scalar
        Variance of the Curie temperatures.
    """
    print "\t Gaussian Distributions of TC's"

    plt.figure()
    plt.plot(T, mag.Gauss(T, TC1, Var1))
    plt.plot(T, mag.Gauss(T, TC2, Var2))
    plt.title('Gaussian Distribution, $\sigma^2_1=$' +
              str(Var1) + ', $\sigma^2_2=$' + str(Var2) + '')
    plt.ylabel('Gaussian Distribution')
    plt.xlabel('T (K)')


def plt_M_gauss_vs_T(T, B, sigma_1_gauss, sigma_2_gauss, Bstep=1, save=False):
    """Plots the magnetization as a function of temperature for several Curie
    temperatures with a gaussian distribution.

    Parameters
    ----------
    T : array
        Array with the temperatures. The array must include both Curie temperatures.
    B : scalar, array
        Magnetic fields.
    sigma_1, sigma_2, sigma_stable : array, 2D array
        Arrays with the values of the respective magnetizations
    ...
    save : bool
        Save the data of this plot to a .txt file.
    """
    print '\t Brillouin_Gaussian as a function of Temperature'

    plt.figure()
    for i in range(0, len(B), Bstep):
        plt.plot(T, sigma_1_gauss[i], label='1, B=' + str(B[i]) + 'T')
        plt.plot(T, sigma_2_gauss[i], label='2, B=' + str(B[i]) + 'T')

    plt.title('Reduced Magnetizations with Gaussian Distribution of $T_C$, $\sigma$')
    plt.legend(loc=0)
    plt.ylim(0, 1.05)
    plt.ylabel('$\sigma(T, B)$')
    plt.xlabel('T (K)')
...
def plt_M_gauss_vs_B(T, B, sigma_1_gauss, sigma_2_gauss, Tstep=1, save=False):
    """Plots the magnetization as a function of temperature for several Curie
    temperatures with a gaussian distribution.
    ...
    plt.figure()
    for i in range(0, len(T), Tstep):
        plt.plot(B, sigma_1_gauss[:, i], label='1, T=' + str(T[i]) + 'K')
        plt.plot(B, sigma_2_gauss[:, i], label='2, T=' + str(T[i]) + 'K')
...
def plt_M_gauss_vs_TB(sigma_gauss):
    """Plots the magnetization as a function of temperature and magnetic field
    for several Curie temperatures with a gaussian distribution.

    Parameters
    ----------
    sigma_gauss : 2D array
        Arrays with the values of the respective magnetizations
    """
    print '\t 2D plot of Reduced Magnetization'

    plt.figure()
    plt.imshow(sigma_gauss, aspect='auto')
    plt.title('Reduced Magnetizations of Phase 1, $\sigma$')
    plt.colorbar()
    plt.ylabel('B (T)')
    plt.xlabel('T (K)')
...
def plt_DeltaS_vs_T(T, B, s_tot, s_M_tot, s_L_tot, Bstep=1, save=False, conv=False):
    ...
    save : bool
        Save the data of this plot to a .txt file.
    conv = bool
        Enables the conversion from eV/K to J/Kg K (or the conversion used in Variables.py).
    """
    if conv == False:  # if the conversion is disabled
        Conv = 1.

    plt.figure()
    for i in range(1, len(B), Bstep):
        plt.plot(T, Conv * (s_tot[i] - s_tot[0]), label='$\Delta S^{Tot}($' + str(
            B[0]) + '$\longrightarrow$' + str(B[i]) + ' T$)$')

    plt.gca().set_color_cycle(None)
    for i in range(1, len(B), Bstep):
        plt.plot(T, Conv * (s_M_tot[i] - s_M_tot[0]), '--', label='$\Delta S^{M}($' + str(
            B[0]) + '$\longrightarrow$' + str(B[i]) + ' T$)$')

    plt.gca().set_color_cycle(None)
    for i in range(1, len(B), Bstep):
        plt.plot(T, Conv * (s_L_tot[i] - s_L_tot[0]), ':', label='$\Delta S^{L}($' + str(
            B[0]) + '$\longrightarrow$' + str(B[i]) + ' T$)$')

        if save == True:
            if not os.path.exists("DeltaS_vs_T"):
                os.makedirs("DeltaS_vs_T")
            zipped = zip(T, Conv * (s_tot[i] - s_tot[0]), Conv * (
                s_M_tot[i] - s_M_tot[0]), Conv * (s_L_tot[i] - s_L_tot[0]))
            np.savetxt("DeltaS_vs_T\DeltaS_vs_T(" + str(B[i]) + "T).txt", zipped, delimiter=',',
                       header='Temperature, Total Entropy Change, Magnetic Entropy Change, Lattice Entropy Change',)
...
def plt_max_DeltaS_vs_B(B, s_tot, save=False, conv=False):
    ...
    if save == True:
        ...
        np.savetxt("max_DeltaS_vs_B\max_DeltaS_vs_B.txt", zipped, delimiter=',',
    ...
    for j in range(0, len(B), Bstep):
        ...
        for i in range(0, len(T), Tstep):
            ...
            if save == True:
                ...
                np.savetxt("S_M_vs_M\S_M_vs_M(" + str(T[i]) + "K," + str(
                    B[j]) + "T).txt", zipped, delimiter=',', header='Reduced Magnetization, Entropy 1, Entropy 2',)
...
def plt_F_vs_T(T, B, free_ener_1, free_ener_2, free_stable, Bstep=1, save=False):
    ...
    for i in range(0, len(B), Bstep):
        ...
        if save == True:
            ...
            np.savetxt("F_vs_T\F_vs_T(" + str(B[i]) + "T).txt", zipped, delimiter=',',
                       header='Temperature, Free Energy 1, Free Energy 2, Free Enrgy Stable',)
...
def plt_F_vs_B(T, B, free_ener_1, free_ener_2, free_stable, Tstep=1, save=False):
    """Plots the free energies of both structures as functions of magnetic field.
...
def plt_F_M_vs_M(sigma, T, B, J1, TC1, lamb1, J2, TC2, lamb2, gJ, Nm, Tstep=1, Bstep=1, save=False):
    """Plots the magnetic free energy as a function of the reduced magnetization.
    ...
    for j in range(0, len(Delta_B), Bstep):
        ...
        plt.legend(loc=0, fontsize='small')
        plt.title(
            'Magnetic Free Energy as a function of Magnetization $F^M$, B=' + str(Delta_B[j]) + 'T')
        plt.xlabel('$\sigma$')
        plt.ylabel('F(T) (eV)')
        plt.xlim(-1, 1)


def plt_F_M_vs_T(T, B, J1, J2, TC1, TC2, lamb1, lamb2, gJ, Nm, Bstep=1, save=False):
    """Plots the free energies of both structures as functions of temperature.
    ...
    for i in range(0, len(Delta_B), Bstep):
        ...
        if save == True:
            ...
            np.savetxt("F_M_vs_T\F_M_vs_T(" + str(Delta_B[i]) + "T).txt", zipped,
                       delimiter=',', header='Temperature, Free Energy 1, Free Energy 2',)
...
def plt_Ftot_vs_M(sigma, T, B, J1, J2, TC1, TC2, lamb1, lamb2, theta_D1,
                  theta_D2, F01, F02, gJ, Nm, N, Tstep=1, Bstep=1,  save=False):
    """Plots the total free energy of both structures as functions of magnetization.
    ...
    for j in range(0, len(B), Bstep):
        ...
        plt.gca().set_color_cycle(None)
        for i in range(0, len(T), Tstep):
            f3 = free.F_totstable_vs_M(sigma, T[i], B[j], J1, TC1, lamb1, theta_D1,
                                       F01, J2, TC2, lamb2, theta_D2, F02, gJ, Nm, N)
            ...
            if save == True:
                ...
                np.savetxt("Ftot_vs_M\Ftot_vs_M(" + str(T[i]) + "K," + str(B[j]) + "T).txt", zipped, delimiter=',',
                           header='Reduced Magnetization, Free Energy 1, Free Energy 2, Stable Free Energy',)

        plt.legend(loc=0, fontsize='small')
        plt.title(
            'Total Free Energy as a function of Magnetization $F^{Tot}=F^M+F^L+F^E$, B=' + str(B[j]) + 'T')
        plt.xlabel('$\sigma$')
        plt.ylabel('F(T) (eV)')
        plt.xlim(-1, 1)
...
def plt_M(MvsT=0, MvsB=0, MvsTB=0, UvsT=0, M_hys_vs_T=0, save=0,
          TT=None, BB=None, J1=None, J2=None, TC1=None, TC2=None, lamb1=None, lamb2=None,
          Delta_T=None, Delta_B=None, theta_D1=None, theta_D2=None, gJ=None,
          F01=None, F02=None, Nm=None, N=None):
    ...
    M_hys_vs_T : bool
        Plots the hysteresis of the magnetization vs tempeperature, magnetic field and both.
    """
    print 'Magnetization'
    if MvsT == True or MvsB == True or UvsT == True:
        sig_1 = mag.Brillouin(TT, BB, J1, gJ, TC1, lamb1,
                              Nm)  # magnetization of phase 1
        sig_2 = mag.Brillouin(TT, BB, J2, gJ, TC2, lamb2,
                              Nm)  # magnetization of phase 2
        sig_stable = mag.Brillouin_stable(TT, BB, J1, J2, TC1, TC2, lamb1, lamb2,
                                          theta_D1, theta_D2, F01, F02, gJ, Nm, N)  # magnetization of stable phase
        if MvsT == True:  # plot of M vs T
            plt_M_vs_T(Delta_T, Delta_B, sig_1, sig_2,
                       sig_stable, Bstep=1, save=save)

        if MvsB == True:  # plot of M vs B
            plt_M_vs_B(Delta_T, Delta_B, sig_1, sig_2,
                       sig_stable, Tstep=5, save=save)

        if MvsTB == True:  # plot of M vs T and B
            plt_M_vs_TB(Delta_T, Delta_B, sig_stable)

        if UvsT == True:  # plot of U vs T
    ...
    if M_hys_vs_T == True:
        ...
        sig_heat = mag.RedMag_heat(TT, BB, J1, TC1, lamb1, theta_D1, F01, J2,
                                   TC2, lamb2, theta_D2, F02, gJ, Nm, N)  # magnetization on heating
        sig_cool = mag.RedMag_cool(TT, BB, J1, TC1, lamb1, theta_D1, F01, J2,
                                   TC2, lamb2, theta_D2, F02, gJ, Nm, N)  # magnetization on cooling
...
def plt_M_gauss(gauss=0, MvsT=0, MvsB=0, MvsTB=0, save=0):
    """Menu for the magnetization plots with a gaussian distribution of Curie 
    temperatures.
    ...
    MvsTB : bool
        Plots the magnetization vs temperature and magnetic field.
    """
    print 'Brillouin Gauss'

    if gauss == True:
        # plot of gaussian distribution of the Curie temperatures
        plt_Gauss(Delta_T, TC1, Var1, TC2, Var2)

    if MvsT == True or MvsB == True or MvsTB == True:
        sig_1_gauss = mag.Brillouin_Gauss(TT, BB, J1, TC1, lamb1, Var1)
        sig_2_gauss = mag.Brillouin_Gauss(TT, BB, J2, TC2, lamb2, Var2)

        if MvsT == True:  # plot of magnetization_gauss vs T
            plt_M_gauss_vs_T(Delta_T, Delta_B, sig_1_gauss,
                             sig_2_gauss, Bstep=1, save=save)

        if MvsB == True:  # plot of magnetization_gauss vs B
            plt_M_gauss_vs_B(Delta_T, Delta_B, sig_1_gauss,
                             sig_2_gauss, Tstep=10, save=save)

        if MvsTB == True:  # plot of magnetization_gauss vs T and B
            plt_M_gauss_vs_TB(sig_1_gauss)
...
def plt_S(S_M_vs_T=0, S_L_VS_T=0, S_tot_vs_T=0, DeltaS_vs_T=0,
    ...
    if S_tot_vs_T == True or DeltaS_vs_T == True or max_DeltaS_vs_B == True:
...
def plt_F(FvsT=0, FvsB=0, trans_temp=0, F_M_vs_T=0, F_M_vs_M=0, F_L_vs_T=0, FtotvsM=0, Ftot_heatcool=0, save=0,
          TT=None, BB=None, J1=None, J2=None, TC1=None, TC2=None, lamb1=None, lamb2=None,
          Delta_T=None, Delta_B=None, theta_D1=None, theta_D2=None, gJ=None,
          F01=None, F02=None, Nm=None, N=None, sigma=None):
    ...
    Ftot_heatcool : bool
        Plots the free energies following the metastable minimums on heating and cooling.
    """
    print 'Free Energies'

    if FvsT == True or FvsB == True:
        ...
        if trans_temp == True:
            plt_transition_temp(TT, BB, save, J1, TC1, theta_D1, F01, lamb1,
                                J2, TC2, theta_D2, F02, lamb2, gJ, Nm, N, free_ener_1, free_ener_2)

    # if the free energies are not already calculated
    if trans_temp == True and (FvsT != True and FvsB != True):
        plt_transition_temp(TT, BB, save, J1, TC1, theta_D1,
                            F01, lamb1, J2, TC2, theta_D2, F02, lamb2, gJ, Nm, N)
    ...
    if FtotvsM == True:
        plt_Ftot_vs_M(sigma, Delta_T, Delta_B, J1, J2, TC1, TC2, lamb1, lamb2,
                      theta_D1, theta_D2, F01, F02, gJ, Nm, N, Tstep=20, Bstep=1, save=save)

    if Ftot_heatcool == True:
        f_heat = free.F_tot_stable_Heating(TT, BB, J1, TC1, lamb1, theta_D1, F01,
                                           J2, TC2, lamb2, theta_D2, F02, gJ, Nm, N)
        f_cool = free.F_tot_stable_Cooling(TT, BB, J1, TC1, lamb1, theta_D1, F01,
                                           J2, TC2, lamb2, theta_D2, F02, gJ, Nm, N)
...
if __name__ == "__main__":
    ...
    plt_M(MvsT=0, MvsB=0, MvsTB=0, UvsT=0, M_hys_vs_T=0, save=0)
    plt_M_gauss(MvsT=0, MvsB=0, MvsTB=0)
    plt_S(S_M_vs_T=0, S_L_VS_T=0, S_tot_vs_T=0, DeltaS_vs_T=0,
          max_DeltaS_vs_B=0, S_M_vs_M=0, save=0)
    plt_F(FvsT=0, FvsB=0, trans_temp=1, F_M_vs_T=0, F_M_vs_M=0,
